...
제공된 예시 문제(problem.md 스타일)와 유사한 난이도와 길이를 가진 '새로운 심층 면접 문제'를 출제해야 합니다.

**중요: 제시문(CONTEXT)은 반드시 길고 구체적이어야 합니다.**
- 단문의 제시문은 절대 안 됩니다.
- 최소 800자 이상의 분량으로 작성하세요.
- 필요하다면 [제시문 가], [제시문 나], [제시문 다] 와 같이 여러 관점을 포함하는 복합 지문 형태로 구성하세요.
- 연세대 특화 키워드(중입자 치료, DX, 의사-과학자 등)를 깊이 있게 다루세요.
...
출력 포맷:
TITLE: [제목]
CONTEXT: [제시문 가], [제시문 나] 등을 포함한 긴 텍스트
QUESTION_LIST:
- [질문 1]
- [질문 2]
KEY_POINTS:
...
def generate_dynamic_question(api_key, topic):
    ...
    [기출 문제 수준 참고]
    제목: {example['title']}
    내용(일부): {example['context'][:500]}... (실제로는 훨씬 깁니다)
    질문: {example['questions']}
    
    위 수준에 맞춰 '{topic}'에 관한 새로운 문제를 출제하세요.
    **제시문은 반드시 3문단 이상, 혹은 [가], [나] 등으로 구성된 긴 글이어야 합니다.**
    """
...
def evaluate_interview(api_key, messages, question_data):
    ...
    ***중요: 평가를 매우 냉정하게 하세요.***
    ...
    3. **종합 총평 및 고득점을 위한 조언 (Punchline Advice)**
       - 강점과 약점 분석
       - **[고득점 전략 Punchline]**: "이 질문에 대한 답변에 반드시 포함되었으면 좋았을 결정적 한 문장(Golden Sentence)"을 예시로 작성해주세요. 추상적인 조언 대신 **구체적인 멘트**를 적어주세요. (예: "제시문 [가]의 입장에서 볼 때, 중입자 치료의 정밀성은 환자의 삶의 질을 높이는 핵심적인 윤리적 가치라고 생각합니다.")
